...
*Setup instructions for a Gitian build of Phantom Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Phantom
...
- [Building Phantom Core](#building-phantom-core)
...
Clone the git repositories for phantom and Gitian.
...
```bash
sudo cp /home/debian/phantom/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the phantom repository under 'Fetch and create inputs' to install sources which require
...
To build Phantom Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the phantom repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/phantom/.git/
...
For example:
```bash
URL=https://github.com/laanwj/phantom.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit phantom=${COMMIT} --url phantom=${URL} ../phantom/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit phantom=${COMMIT} --url phantom=${URL} ../phantom/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit phantom=${COMMIT} --url phantom=${URL} ../phantom/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-phantom:
./bin/gbuild --commit phantom=${COMMIT},cpp-eth-phantom=${ETHCOMMIT} --url phantom=${URL},cpp-eth-phantom=${ETHURL} ../phantom/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the phantom git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../phantom/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/phantom
SIGPATH=/some/root/path/phantom-detached-sigs

./bin/gbuild --url phantom=${BTCPATH},signature=${SIGPATH} ../phantom/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/phantom-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/phantom-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/phantom-osx-build.assert
```
...
[phantomproject/gitian.sigs](https://github.com/phantomproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@phantom.org) and he will commit them.
