...
{
    if (pindexBestHeader->nHeight >= Params().GetConsensus().nLastPOWBlock)//toniphantom edit
        return 0;
...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // phantom
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // phantom
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // phantom
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // phantom
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CBitcoinAddress phantomSenderAddress(request.params[2].get_str());
        if(phantomSenderAddress.IsValid()){
            CKeyID keyid;
            phantomSenderAddress.GetKeyID(keyid);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of phantom addresses\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "blockchain",         "callcontract",           &callcontract,           true,  {"address","data"} }, // phantom
