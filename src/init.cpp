...
void Shutdown()
{
    ...
    if (pwalletMain)
    {
        StakePhantoms(false, pwalletMain);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/phantomproject/phantom>";
    const std::string URL_WEBSITE = "<https://phantom.org>";
...
//////////////////////////////////////////////////////////////////////
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "statePhantom";
                ...
                boost::filesystem::path phantomStateDir = GetDataDir() / "statePhantom";

                bool fStatus = boost::filesystem::exists(phantomStateDir);
                const std::string dirPhantom(phantomStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsPhantomstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<PhantomState>(new PhantomState(dev::u256(0), PhantomState::openDB(dirPhantom, hashDB, dev::WithExisting::Trust), dirPhantom, existsPhantomstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::phantomMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(phantomStateDir.string());
    ...
    if (!GetBoolArg("-staking", DEFAULT_STAKE))
        ...
        StakePhantoms(true, pwalletMain);
#endif
    // ********************************************************* Step 12: finished
