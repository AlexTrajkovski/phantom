...
void checkResult(bool isCreation, std::vector<PhantomTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    PhantomTxConverter converter(transaction, NULL);
    ExtractPhantomTX phantomTx;
    BOOST_CHECK(converter.extractionPhantomTransactions(phantomTx));
    std::vector<PhantomTransaction> result = phantomTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(phantomtxconverter_tests, TestingSetup)
