...
bool ToHash160(const std::string& strPhantomAddress, std::string& strHash160)
{
    CBitcoinAddress phantomAddress(strPhantomAddress);
    if(phantomAddress.IsValid()){
        CKeyID keyid;
        phantomAddress.GetKeyID(keyid);
...
bool ToPhantomAddress(const std::string& strHash160, std::string& strPhantomAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress phantomAddress;
    phantomAddress.Set(keyid);
    if(phantomAddress.IsValid()){
        strPhantomAddress = phantomAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToPhantomAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToPhantomAddress(tokenEvent.receiver, tokenEvent.receiver);
