...
PhantomState::PhantomState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = PhantomState::openDB(_path + "/phantomDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

PhantomState::PhantomState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute PhantomState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, PhantomTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createPhantomAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            phantom::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> PhantomState::vins() const // temp
{
...
void PhantomState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* PhantomState::vin(dev::Address const& _a) const
{
    return const_cast<PhantomState*>(this)->vin(_a);
}

Vin* PhantomState::vin(dev::Address const& _addr)
{
...
// void PhantomState::commit(CommitBehaviour _commitBehaviour)
// {
...
void PhantomState::kill(dev::Address _addr)
{
...
void PhantomState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address PhantomState::createPhantomAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void PhantomState::deleteAccounts(std::set<dev::Address>& addrs){
...
void PhantomState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void PhantomState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
