...
#if defined(NDEBUG)
# error "Phantom cannot be compiled without assertions."
#endif
...
std::unique_ptr<PhantomState> globalState;
...
const std::string strMessageMagic = "Phantom Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // phantom
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            PhantomDGP phantomDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = phantomDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = phantomDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            PhantomTxConverter converter(tx, NULL);
            ExtractPhantomTX resultConverter;
            if(!converter.extractionPhantomTransactions(resultConverter)){
            ...
            std::vector<PhantomTransaction> phantomTransactions = resultConverter.first;
            std::vector<EthTransactionParams> phantomETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(PhantomTransaction phantomTransaction : phantomTransactions){
                sumGas += phantomTransaction.gas() * phantomTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, phantomTransaction.gasPrice());
                } else {
                    txMinGasPrice = phantomTransaction.gasPrice();
                }
                VersionVM v = phantomTransaction.getVersion();
                if(v.format!=0)
                ...
                if(phantomTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(phantomTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(phantomTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += phantomTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)phantomTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(phantomETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > phantomTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s XPTg additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // phantom
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // phantom
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    PhantomTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<PhantomTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path phantomDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(phantomDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(phantomDir.string(), std::ios::in | std::ios::out);
...
bool PhantomTxConverter::extractionPhantomTransactions(ExtractPhantomTX& phantomtx){
    std::vector<PhantomTransaction> resultTX;
    ...
    phantomtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool PhantomTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool PhantomTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
PhantomTransaction PhantomTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    PhantomTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = PhantomTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = PhantomTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // phantom
    PhantomDGP phantomDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setPhantomSchedule(phantomDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = phantomDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = phantomDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = phantomDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // phantom
...
///////////////////////////////////////////////////////////////////////////////////////// phantom
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            PhantomTxConverter convert(tx, &view, &block.vtx);

            ExtractPhantomTX resultConvertPhantomTX;
            if(!convert.extractionPhantomTransactions(resultConvertPhantomTX)){
            ...
            if(!CheckMinGasPrice(resultConvertPhantomTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertPhantomTX.first, blockGasLimit);
            ...
            for(PhantomTransaction& qtx : resultConvertPhantomTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertPhantomTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertPhantomTX.first[k].from(), resultConvertPhantomTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // phantom
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // phantom
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // phantom
            globalState->setRootUTXO(oldHashUTXORoot); // phantom
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // phantom
        uint32_t sizeBlockDGP = phantomDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Phantom PoS we need to sync this after every block to ensure txdb is populated for
