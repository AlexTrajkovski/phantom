...
/////////////////////////////////////////// phantom
#include <phantom/phantomstate.h>
#include <phantom/phantomDGP.h>
...
using ExtractPhantomTX = std::pair<std::vector<PhantomTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // phantom
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashPhantomRoot,
    const dev::h256& oldHashStateRoot, const std::vector<PhantomTransaction>& transactions);
...
class PhantomTxConverter{

public:

    PhantomTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionPhantomTransactions(ExtractPhantomTX& phantomTx);
...
private:
    ...
    PhantomTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<PhantomTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
