#include "phantomDGP.h"

void PhantomDGP::initDataEIP158(){
...
bool PhantomDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule PhantomDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t PhantomDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t PhantomDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t PhantomDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t PhantomDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool PhantomDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void PhantomDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void PhantomDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void PhantomDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void PhantomDGP::createParamsInstance(){
...
dev::Address PhantomDGP::getAddressForBlock(unsigned int blockHeight){
...
void PhantomDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void PhantomDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void PhantomDGP::parseStorageOneUint64(uint64_t& value){
...
void PhantomDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule PhantomDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
